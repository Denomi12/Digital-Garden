...
public class MapRasterTiles {
    ...
    static String token = "?&apiKey=" + si.um.feri.mori.raster.utils.Keys.GEOAPIFY;
    ...
    public static Vector2 getPixelPosition(double lat, double lng, int beginTileX, int beginTileY) {
        ...
        // Scale to fit our image
        double scale = Math.pow(2, si.um.feri.mori.raster.utils.Constants.ZOOM);

        // Apply scale to world coordinates to get image coordinates
        return new Vector2(
                (int) (Math.floor(worldCoordinate[0] * scale) - (beginTileX * MapRasterTiles.TILE_SIZE)),
                si.um.feri.mori.raster.utils.Constants.MAP_HEIGHT - (int) (Math.floor(worldCoordinate[1] * scale) - (beginTileY * MapRasterTiles.TILE_SIZE) - 1)
        );
    }

    public static si.um.feri.mori.raster.utils.Geolocation[][] fetchPath(si.um.feri.mori.raster.utils.Geolocation[] geolocations){
    ...
    public static si.um.feri.mori.raster.utils.Geolocation[][] getRouteFromCoordinates(double[][] coordinates) throws Exception {
        ...
        String urlString = "https://api.geoapify.com/v1/routing?waypoints=" + coordinatesPath.toString() +
                "&mode=" + "drive" + "&apiKey=" + si.um.feri.mori.raster.utils.Keys.GEOAPIFY;
        ...
        si.um.feri.mori.raster.utils.Geolocation[][] geolocations;
        ...
        if (features.length() > 0) {
            ...
            geolocations = new si.um.feri.mori.raster.utils.Geolocation[coordinatesArray.length()][];
            // Print each coordinate in the path
            for (int i = 0; i < coordinatesArray.length(); i++) {
                JSONArray coord = coordinatesArray.getJSONArray(i);

                si.um.feri.mori.raster.utils.Geolocation[] geol = new si.um.feri.mori.raster.utils.Geolocation[coord.length()];
                for (int j = 0; j < coord.length(); j++) {
                    JSONArray c = coord.getJSONArray(j);
                    double lon = c.getDouble(0);
                    double lat = c.getDouble(1);
                    System.out.println("Longitude: " + lon + ", Latitude: " + lat);
                    geol[j] = new si.um.feri.mori.raster.utils.Geolocation(lat, lon);
                }
                geolocations[i] = geol;
            }
